...
const SplitScreenTemplate = ({ data, accentColor, accentBg, fontSize, sectionSpacing }) => {
        ...
        <div className="w-full min-h-full flex" style={{ fontSize: fontSize || 15 }}>
            ...
            <aside className="w-[36%] text-white p-7 flex flex-col gap-6 shrink-0" style={{ background: accentBg || accentColor }}>
                ...
                <div className="text-center">
                    <h1 className="text-2xl font-bold tracking-wide leading-tight">{data.personal_info?.full_name || "Your Name"}</h1>
                    {data.experience?.[0]?.position && (
                        <p className="text-xs mt-1 uppercase tracking-widest text-white/70">{data.experience[0].position}</p>
                    )}
                </div>

                {/* Summary */}
                {data.professional_summary && (
                    <p className="text-xs leading-relaxed text-white/80 text-center italic">
                        "{data.professional_summary}"
                    </p>
                )}
                ...
                {/* Contact */}
                <div>
                    <h2 className="text-[10px] font-bold uppercase tracking-widest border-b border-white/20 pb-1.5 mb-3">Contact</h2>
                ...
                {data.education?.length > 0 && (
                    <div>
                        <h2 className="text-[10px] font-bold uppercase tracking-widest border-b border-white/20 pb-1.5 mb-3">Education</h2>
                        <div className="space-y-3">
                            {data.education.map((edu, i) => (
                                <div key={i}>
                                    <p className="font-semibold text-xs">{edu.degree}</p>
                                    <p className="text-[11px] text-white/70">{edu.institution}</p>
                                    <p className="text-[10px] text-white/50">{formatDate(edu.graduation_date)}</p>
                                    {edu.gpa && <p className="text-[10px] text-white/50">GPA: {edu.gpa}</p>}
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {data.skills?.length > 0 && (
                    <div>
                        <h2 className="text-[10px] font-bold uppercase tracking-widest border-b border-white/20 pb-1.5 mb-3">Skills</h2>
                        <div className="flex flex-wrap gap-1.5">
                            {data.skills.map((skill, i) => (
                                <span key={i} className="bg-white/15 border border-white/10 px-2 py-0.5 rounded text-[11px]">{skill}</span>
                            ))}
                        </div>
                    </div>
                )}
            ...
            <main className="flex-1 bg-white text-gray-800 p-8 flex flex-col gap-5">
                ...
                {data.experience?.length > 0 && (
                    <section style={{ marginBottom: sectionSpacing || 24 }}>
                        <h2 className="text-lg font-bold uppercase tracking-widest mb-5 pb-2 border-b-2" style={{ color: accentColor, borderColor: accentColor }}>
                            Experience
                        </h2>
                            ...
                            {data.project.map((p, i) => (
                                <div key={i} className="pl-4 border-l-2" style={{ borderColor: `${accentColor}40` }}>
                                    <div className="flex items-center gap-2">
                                        ...
                                        {p.link && <a href={p.link} target="_blank" rel="noopener noreferrer" className="text-xs hover:underline" style={{ color: accentColor }}>↗ Link</a>}
                                    </div>
                                    {p.type && <p className="text-[11px] text-gray-400">{p.type}</p>}
