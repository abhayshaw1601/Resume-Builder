...
const InfographicTemplate = ({ data, accentColor, accentBg, fontSize, sectionSpacing }) => {
        ...
        <div className="max-w-4xl mx-auto bg-gradient-to-br from-gray-50 to-white text-gray-800 p-8" style={{ fontSize: fontSize || 14 }}>
            ...
            <header className="text-center mb-6 pb-6 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                {data.experience?.[0]?.position && (
                    <p className="text-sm font-semibold mb-3" style={{ color: accentColor }}>{data.experience[0].position}</p>
                )}

                {/* Contact Pills */}
                <div className="flex flex-wrap justify-center gap-2 mt-3">
                    {data.personal_info?.email && (
                        <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs" style={{ backgroundColor: `${accentColor}12`, color: accentColor }}>
                            <Mail size={11} />{data.personal_info.email}
                        </span>
                    )}
                    {data.personal_info?.phone && (
                        <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs bg-emerald-50 text-emerald-700">
                            <Phone size={11} />{data.personal_info.phone}
                        </span>
                    )}
                    {data.personal_info?.location && (
                        <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs bg-amber-50 text-amber-700">
                            <MapPin size={11} />{data.personal_info.location}
                        </span>
                    )}
                    {data.personal_info?.linkedin && (
                        <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs bg-blue-50 text-blue-700">
                            <Link size={11} />LinkedIn
                        </span>
                    )}
                    {data.personal_info?.website && (
                        <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs bg-purple-50 text-purple-700">
                ...
                {data.professional_summary && (
                    <p className="text-sm text-gray-600 leading-relaxed mt-4 max-w-2xl mx-auto">{data.professional_summary}</p>
                )}
            </header>
            ...
            {data.skills?.length > 0 && (
                <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-4" style={{ marginBottom: sectionSpacing || 16 }}>
                    <h2 className="text-sm font-bold uppercase tracking-widest mb-4 text-center" style={{ color: accentColor }}>
                        Skill Proficiency
                    </h2>
