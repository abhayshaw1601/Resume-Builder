...
const AcademicCVTemplate = ({ isDarkMode, data, accentColor, accentBg, fontSize, headingSize, sectionSpacing }) => {
    ...
    const SectionTitle = ({ children }) => (
        <h2 className={`font-bold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'} pb-1.5 mb-4 border-b-2`} style={{ borderColor: accentColor, fontFamily: "'Georgia', 'Times New Roman', serif", fontSize: headingSize ? headingSize * 0.6 : 14 }}>
            {children}
        </h2>
    );

    return (
        <div className={`max-w-4xl mx-auto ${isDarkMode ? 'bg-[#0a0a0a]' : 'bg-white'} ${isDarkMode ? 'text-gray-200' : 'text-gray-800'} p-10`} style={{ fontSize: fontSize || 15, fontFamily: "'Georgia', 'Times New Roman', serif" }}>
            {/* Header */}
            <header className="text-center pb-5 mb-6 border-b-2 border-gray-800">
                <h1 className={`font-bold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'} tracking-wide`} style={{ fontSize: headingSize || 24, fontFamily: "'Georgia', 'Times New Roman', serif" }}>
            ...
            {data.professional_summary && (
                <section style={{ marginBottom: sectionSpacing || 24 }}>
                    ...
                    <p className={`${isDarkMode ? 'text-gray-300' : 'text-gray-700'} leading-relaxed text-[0.875em]`}>{data.professional_summary}</p>
                </section>
            )}
                    ...
                    <div className="space-y-4">
                        {data.experience.map((exp, i) => (
                            <div key={i} className={`pb-3 border-b ${isDarkMode ? 'border-white/10' : 'border-gray-200'} last:border-0 last:pb-0`}>
                                ...
                                {exp.description && (
                                    <ul className={`mt-1.5 space-y-0.5 text-[0.875em] ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} list-disc list-inside`} style={{ fontFamily: "system-ui, sans-serif" }}>
            ...
            {data.skills?.length > 0 && (
                <section style={{ marginBottom: sectionSpacing || 24 }}>
                    ...
                    <div className={`text-[0.875em] ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`} style={{ fontFamily: "system-ui, sans-serif" }}>
                        {data.skills.join(" • ")}
                    </div>
                </section>
            )}
