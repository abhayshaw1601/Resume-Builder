...
const ModernMonoTemplate = ({ data, accentColor, accentBg, fontSize, sectionSpacing }) => {
    ...
    return (
        <div className="max-w-4xl mx-auto bg-white text-gray-800 p-10" style={{ fontSize: fontSize || 15 }}>
            ...
            <header className="text-center pb-8 mb-8 border-b border-gray-200">
                ...
                <div className="flex justify-center flex-wrap gap-5 text-xs text-gray-500 mt-4">
            ...
            {data.experience?.length > 0 && (
                <section style={{ marginBottom: sectionSpacing || 32 }}>
                    <h2 className="text-lg font-light tracking-widest uppercase mb-6" style={{ color: accentColor }}>
                        Experience
                    </h2>
                        ...
                        {data.experience.map((exp, i) => (
                            <div key={i}>
                                <div className="flex justify-between items-baseline">
                                    ...
                                    <span className="text-xs text-gray-400 font-light whitespace-nowrap ml-4">
                                ...
                                {exp.description && (
                                    <div className="mt-2 text-sm text-gray-600 leading-relaxed font-light whitespace-pre-line">
                                        {exp.description}
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="space-y-4">
                        {data.project.map((p, i) => (
                            <div key={i}>
                                ...
                                {p.type && <p className="text-xs text-gray-400 font-light">{p.type}</p>}
                                {p.description && <p className="text-sm text-gray-600 font-light mt-1">{p.description}</p>}
                            </div>
                        ))}
                    </div>
                </section>
            ...
            <div className="grid grid-cols-2 gap-10 pt-6 border-t border-gray-200">
                ...
                {data.education?.length > 0 && (
                    <section>
                        <h2 className="text-lg font-light tracking-widest uppercase mb-5" style={{ color: accentColor }}>
                            Education
                        </h2>
                    ...
                    <section>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {data.skills.map((skill, i) => (
                                <span key={i} className="text-sm text-gray-600 font-light">
